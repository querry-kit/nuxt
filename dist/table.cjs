...
function useTable(options) {
  ...
  const page = (0, import_vue2.ref)(normalisePage(options.routePage?.value) ?? 1);
  ...
  const totalItems = (0, import_vue2.ref)(0);
  const totalPages = (0, import_vue2.ref)(0);
  const loading = (0, import_vue2.ref)(false);
  const error = (0, import_vue2.ref)();
  let requestVersion = 0;
  const availableColumns = (0, import_vue2.computed)(() => options.columns.value.filter(hasColumnId));
  ...
  (0, import_vue2.watch)(availableColumns, syncColumnOrder, { immediate: true, deep: true });
  ...
  const fields = (0, import_vue2.computed)(() => {
  ...
  const staticWhere = (0, import_vue2.computed)(() => {
    const archive = options.isArchived === void 0 ? void 0 : { isArchived: options.isArchived };
    return andWhere(archive, (0, import_vue2.unref)(options.staticFilter));
  });
  const where = (0, import_vue2.computed)(() => andWhere(staticWhere.value, filteringToWhere(filtering.value)));
  const queryParams = (0, import_vue2.computed)(() => ({
    page: page.value,
  ...
  const updateRow = (row) => {
    ...
    if (index !== -1) {
      ...
      (0, import_vue2.triggerRef)(items);
    }
  };
  (0, import_vue2.watch)([queryParams, fields], () => void refresh(), { deep: true });
  return {
    page,
    itemsPerPage,
    sorting,
